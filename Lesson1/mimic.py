...
def mimic_dict(filename):
    ...
    words = data.split();
    dict[''] = [words[0]];
    for i in range(len(words)-1): #borne de droite exclue
        if words[i] in dict:
            dict[words[i]].append(words[i+1]);
        else:
            dict[words[i]] = [words[i+1]];
    if words[len(words)-1] in dict:
        dict[words[len(words)-1]].append('');
    else:
        dict[words[len(words)-1]] = [''];
